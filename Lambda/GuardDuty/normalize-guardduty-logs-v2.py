...
# -> 공통 필드 추출 (CloudTrail 정규화와 동일한 스키마 2.0)
# -> Log 출력
# -> S3에 저장
#
...
# 어느 서비스가 만든 이벤트인지 나타내는 식별자다
# (CloudTrail 정규화의 event.service 자리와 같은 용도)
...
# 정규화 결과를 넘길 룰 평가 Lambda 이름 (CloudTrail 정규화와 같은 함수를 쓴다)
...
def normalize_resource(detail, account_id, region):
  """
  Finding의 resource 블록을 CloudTrail 정규화와 같은 리소스 배열 형태로 변환한다.

  CloudTrail은 resources 배열을 그대로 쓰지만 GuardDuty는 resourceType에 따라
  accessKeyDetails / instanceDetails 처럼 서로 다른 키에 대상이 흩어져 있어
  공통 구조로 직접 재조립해야 한다.
  """
...
def normalize(record, collection_path, collection_source, collector,
  ...
  return{
    ...
    "event":{
      "id": event_id,
      # updatedAt이 파인딩이 마지막으로 갱신된 시각이다 (봉투의 time은 전달 시각)
      "time": detail.get("updatedAt") or record.get("time"),
      "ingested_at": utc_now(),
    ...
    "actor":{
      "type": access_key_details.get("userType"),
      "account_id": account_id,
    ...
    "resources": normalize_resource(detail, account_id, region),
    ...
    "details":{
      ...
      "observed_api":{
        ...
        "service_name": api_call_action.get("serviceName"),
        "api": api_call_action.get("api"),
        "caller_type": api_call_action.get("callerType"),
      },

      "remote_ip":{
        "country": (remote_ip_details.get("country") or {}).get("countryName"),
        "city": (remote_ip_details.get("city") or {}).get("cityName"),
        "organization": (remote_ip_details.get("organization") or {}).get("org"),
      },

      "target_resource":{
        "resource_type": resource.get("resourceType"),
        # 탈취된 자격 증명이 속한 역할 이름
        "role_name": access_key_details.get("userName"),
        "instance_id": instance_details.get("instanceId"),
        ...
        "iam_instance_profile_arn": (
          (instance_details.get("iamInstanceProfile") or {}).get("arn")
        ),
      },
    }
  }
...
def save_normalized_events(normalized_events):
  # 정규화된 파인딩을 이벤트 ID 별로 S3에 저장한다. 파인딩 ID를 객체 키로 사용하므로
  # 같은 파인딩이 갱신되어 다시 들어와도 동일한 객체를 덮어쓴다
...
def invoke_rules_evaluator(normalized_events):
  ...
  # (detect trail Lambda가 정규화 Lambda를 호출하는 방식과 동일한 비동기 호출)
      ...
      json.dumps(
        {
          "level": "WARNING",
          "message": (
            "RULES_FUNCTION_NAME is not set, skipping rule evaluation"
          ),
        },
        ensure_ascii=False,
      )
  ...
  for normalized_event in normalized_events:
    ...
    # 평가 호출 실패가 이미 끝난 S3 적재를 되돌리지 않도록 이벤트 단위로 처리한다
    except Exception as error:
      print(
        json.dumps(
          {
            "level": "ERROR",
            "message": "Rule evaluation invoke failed",
...
def handle_cloudwatch_logs(event, context):
      ...
      ValueError,
        ...
        print(
          json.dumps(
            {
              "level": "ERROR",
              "message": "GuardDuty Normalization failed",
              "error_message": str(error),
              "log_event_id":(log_event.get("id"))
            },
            ensure_ascii=False,
  ...
  return {
    ...
    "rule_evaluation_invoked_count": invoked_count,
  }
